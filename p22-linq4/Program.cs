...
estudiantes.Add(new Estudiante("5678","Rodrigo Mata","Av Mexico 114","Rio Grande",'H',31,true,new List<float>{65,44,22,55}));
estudiantes.Add(new Estudiante("2356","Miguel Mejía","La Loma 666","Fresnillo",'H',31,true,new List<float>{90,42,71,65}));
estudiantes.Add(new Estudiante("6666","David Monreal","5 de Mayo 23","Rio Grandre",'H',41,false,new List<float>{90,80,60,60}));
...
estudiantes.Add(new Estudiante("999","Bere Diaz","Sierra Mezquite 34","Fresnillo",'M',25,true,new List<float>{34,22,33,22}));
estudiantes.Add(new Estudiante("888","Rocio Bernal","Calle Amargura 66","Fresnilo",'M',25,true,new List<float>{77,66,33,22}));

Console.WriteLine("Todos los estudiantes en el grupo");
...
var estprom1 = (from e in estudiantes select $"Nombre = {e.Nombre,-18} - Prom").ToList(); 
Console.WriteLine($"\nEstudiantes con promedio >= {prom} - {estprom.Count()}");
estprom.ForEach(e=>Console.WriteLine(e));
...
Console.WriteLine($"Total de No Becados = {totb}");
